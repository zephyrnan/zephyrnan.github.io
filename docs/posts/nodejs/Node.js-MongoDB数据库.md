...
>   }
>
>   // 执行查询(注意: 变量名不能和上面的集合 products 重名,否则会报错)
...
// 连接数据库
// 注意: Mongoose 6+ 已不再需要 useNewUrlParser、useUnifiedTopology 等选项,
// 这些选项现在是默认行为,传了也会被忽略
...
> 💡 **常见面试问法：MongoDB 中关联数据用「嵌入(embed)」还是「引用(reference)」？**
> - **嵌入式文档**：把关联数据直接放进同一个文档里。适合「一对少量、关联数据基本只跟着主文档一起读」的场景，如用户的多个收货地址、文章的评论(数量可控时)。优点是一次查询拿到全部数据，无需 join；缺点是文档会变大(单文档上限 16MB)，且嵌入数据更新麻烦。
> - **引用(Population)**：只存对方的 `_id`，查询时用 `.populate()` 关联。适合「一对多/多对多、被关联数据会被独立查询或频繁更新」的场景，如文章的作者、订单的商品。优点是数据不冗余、易维护；缺点是需要额外查询(populate 本质是多次查询)。
> - **经验法则**：读多写少、关联数据「从属」于主文档 → 嵌入；关联数据是独立实体、会被多处复用 → 引用。
...
```javascript
// Mongoose 6+ 的连接池等选项写法(旧版的 poolSize 已改名为 maxPoolSize)
mongoose.connect('mongodb://localhost:27017/myapp', {
  maxPoolSize: 10,       // 连接池最大连接数(旧版叫 poolSize)
  minPoolSize: 2,        // 连接池最小连接数
  socketTimeoutMS: 45000,
  serverSelectionTimeoutMS: 5000,  // 选择服务器超时时间
  family: 4              // 使用 IPv4
});
```
...
const connectDB = async () => {
  try {
    // Mongoose 6+ 推荐的连接选项
    // (useNewUrlParser/useUnifiedTopology/useCreateIndex/useFindAndModify 均已废弃)
    const options = {
      maxPoolSize: 10,
