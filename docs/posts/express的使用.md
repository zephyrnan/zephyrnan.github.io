...
Express的安装可直接使用npm包管理器，在使用npm之前可先配置淘宝镜像加速下载（cnpm 为可选项，直接用 npm 也完全可以）：
...
这样我们可以使用cnpm来代替npm，这使得下载速度提高很多；其次你需要在你项目目录下运行以下指令来初始化npm，期间所有提示按enter键即可，这会生成package.json，它是用于描述项目文件的。
...
安装完成后，项目目录中会多出一个node_modules文件夹，里面就是 express 及其依赖的模块。可以通过查看 package.json 的 dependencies 字段，或运行下面命令确认 express 版本：
...
路由表示应用程序端点 (URI) 的定义以及响应客户端请求的方式。它包含一个请求方法（method）、路径（path）和路由匹配时的回调函数（callback）;

```javascript
app.method(path, callback);  // method 为 get/post/put/delete 等 HTTP 方法
```
...
// 当对首页发起 GET 请求时,响应 "hello world"
app.get('/', function(req, res) {
  // 注意:一次请求只能发送一次响应,写完一个 res.send 后,
  // 后面再调用 res.send 会报错 "Cannot set headers after they are sent"
  // res.send 会根据参数类型自动设置 Content-Type:
  //   res.send([1, 2, 3]);   // 数组 -> JSON
  //   res.send({ ok: 1 });   // 对象 -> JSON
  //   res.json({ ok: 1 });   // 显式发送 JSON
  res.send('hello world');
});

// 发送 HTML 字符串(标签要正确闭合: <h1> 对应 </h1>)
...
从 Express 4.16 起，`express.json()` 和 `express.urlencoded()` 已内置，无需再单独安装 body-parser。**注意：`req.body` 不是方法，是一个对象，应写 `req.body` 而不是 `req.body()`。**

> ⚠️ **新手最常踩的坑：`req.body` 是 `undefined`？**
> 多半是漏了解析中间件，或前端 `Content-Type` 和后端解析方式对不上：
> - 前端发送 JSON（`Content-Type: application/json`）→ 后端必须 `app.use(express.json())`，且前端 `body` 要用 `JSON.stringify(...)` 转成字符串。
> - 前端发送表单（`Content-Type: application/x-www-form-urlencoded`）→ 后端必须 `app.use(express.urlencoded({ extended: true }))`。
> - 解析中间件必须注册在路由**之前**，否则路由先执行时 `req.body` 还没被填充。

请求:

```javascript
// 注意:fetch 不是 fecth;发送 JSON 时 body 必须用 JSON.stringify 转成字符串
...
// 配置解析 post 参数的中间件-无需下载第三方,Express 已内置
// 解析 post 参数-(url-encoded 格式) username=kerwin&password=1234
server.use(express.urlencoded({ extended: false }));
// 解析 post 参数-(json 字符串) {name:"",age:100}
